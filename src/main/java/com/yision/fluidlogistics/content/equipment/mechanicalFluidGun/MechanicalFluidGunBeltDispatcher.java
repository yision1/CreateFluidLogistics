	...
	static ProcessingResult held(TransportedItemStack item, TransportedItemStackHandlerBehaviour handler) {
		...
		handler.handleProcessingOnAllItems(current -> {
			...
			for (var gun : ready) {
				...
				if (!outputs.isEmpty() && outputs.getLast().stack.has(AllDataComponents.SEQUENCED_ASSEMBLY)
					&& filling.refreshAssembly(gun, outputs.getLast().stack)) {
					if (source != null && !filling.drainPendingFluid(source).isEmpty()) {
						outputs.getLast().stack = filling.getPreparedResult().copy();
						completed.add(gun);
					}
					filling.clear();
					continue;
			...
			if (held == null) {
				...
				if (held == null && outputs.size() == 1 && ItemStack.matches(outputs.getFirst().stack, before)) {
					held = outputs.getFirst();
				}
